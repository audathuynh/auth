...
In the step 1, client or user `C` sends C<sub>id</sub> and K<sub>c</sub> to the authentication server `A`.
...
In this step, the client `C` sends C<sub>id</sub>, K<sub>c</sub>, and a nounce n<sub>1</sub> to the server `A`.
The C<sub>id</sub> can be the username or user id of the user `C`.
K<sub>c</sub> can be the password that the user inputted.
n<sub>1</sub> can be a random string or a timestamp generated by the client `C`.
...
In this step, the server `A` verifies C<sub>id</sub> and K<sub>C</sub> in the system. 
If it is successful, the server `A` generates randomly a key K<sub>CS</sub> for the communication between `C` and `S`.
Then the server `A` encrypts the key K<sub>CS</sub> and the nounce n<sub>1</sub> together by using the key K<sub>C</sub>.
The client `C` knows the key K<sub>C</sub>. 
Therefore, the client `C` can decrypt and get the key K<sub>C</sub> in the result. 

Moreover, a ticket T<sub>CS</sub> for the communication between `C` and `S` is also created by using a secrete key K<sub>S</sub>.
The secrete key K<sub>S</sub> is known by the server `S` so that only `S` can decrypt the ticket T<sub>CS</sub> to get the key K<sub>CS</sub> inside the ticket.
Asymetric cryptography can be considered to encrypt and decrypt T<sub>CS</sub> if necessary.

### Step 3: C &rarr; S: {C<sub>id</sub>, n<sub>2</sub>}<sub>K<sub>CS</sub></sub>, T<sub>CS</sub>, request

In this step, the client `C` uses the key K<sub>CS</sub> obtained from the server `A` in the step 2 to encrypt C<sub>id</sub> and a nounce n<sub>2</sub> to send to the server `S`.
The client `C` also sends the ticket T<sub>CS</sub> in the request to the server `S`.
...
The server `S` uses the key K<sub>CS</sub> to decrypt {C<sub>id</sub>, n<sub>2</sub>}<sub>K<sub>CS</sub></sub> to verify C<sub>id</sub> and the nounce n<sub>2</sub> generated by the client `C`.
The server `S` performs the request to have a result. 
The server `S` uses the key K<sub>CS</sub> to encrypt the nounce n<sub>2</sub> to return it to the client in the result.
